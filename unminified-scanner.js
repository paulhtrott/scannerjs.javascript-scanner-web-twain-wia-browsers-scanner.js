!function(global, definition) {
    var forceGlobal = window && ("global_scanner" in window) ? window["global_scanner"] : true;
    ...
    function getScannerJsConfig(key, defaultValue) {
        var keyPrefixed = "scannerjs_" + key;
        var val = window && (keyPrefixed in window) ? window[keyPrefixed] : undefined;
        if (!val && window && ("scannerjs_config" in window)) {
            val = (key in window.scannerjs_config) ? window.scannerjs_config[key] : undefined;
        }
    ...
    var scannerDef = {
        ...
        EVENT_POST_INT: "post-init",
        EVENT_FUNC_CALL: "func-call",
        EVENT_FUNC_CANCELLED: "func-cancelled",
        EVENT_FUNC_RETURN: "func-return",
        EVENT_READY: "ready",
        EVENT_FAILED_TO_CONNECT: "failed-to-connect",
        EVENT_FAILED_TO_CONNECT_FINAL: "failed-to-connect-final",
        EVENT_DISCONNECTED: "disconnected",
        LOG_LEVEL_INFO: 4,
        LOG_LEVEL_ERROR: 16,
        LOG_LEVEL_ALL: 0,
        LOG_LEVEL_OFF: 1024,
        logLevel: getScannerJsConfig("log_level", 0),
        scanAppletEnabled: getScannerJsConfig("java_applet_enabled", false),
        scanAppletCodeBase: undefined,
        scanAppletCodeBaseFallbackIfNoScannerJs: "https://asprise.azureedge.net/scannerjs/",
        scanAppletStarted: false,
        scanAppEnabled: getScannerJsConfig("scan_app_enabled", true),
        scanAppDownloadUrl: getScannerJsConfig("scan_app_download_url", "http://asprise.azureedge.net/scanapp/scan-setup.exe"),
        scanAppMinVersionRequired: getScannerJsConfig("scan_app_min_version", "2.04"),
        scanAppLicense: getScannerJsConfig("license", window.com_asprise_scan_app_license),
        skipLoadDefaultCss: getScannerJsConfig("skip_load_default_css", false),
        scanWebSocketServerRangeLowest: getScannerJsConfig("scan_app_port_range_lowest", 9713),
        scanWebSocketServerRangeHighest: getScannerJsConfig("scan_app_port_range_highest", 9716),
        ...
        formFieldNameForImagesObjects: getScannerJsConfig("form_field_name_for_img_objects", "com_asprise_scannerjs_images[]"),
        formFieldNameForImagesUrls: getScannerJsConfig("form_field_name_for_img_urls", "com_asprise_scannerjs_images_urls[]"),
        isInitialized: false,
        eagerInitialization: getScannerJsConfig("eager_init", true),
        initializationStartedOn: null,
        fileCount: 0,
        displayInstallScanAppFunc: getScannerJsConfig("display_install_func", undefined),
        displayScanReadyPopupFunc: getScannerJsConfig("display_scan_ready_func", undefined),
        scannerJsEventListener: getScannerJsConfig("event_listener", undefined),
        systemResponseCallbackFunc: getScannerJsConfig("sys_response_callback_func", undefined),
        isReady: false,
        additionalCssClassesForDialogs: getScannerJsConfig("additional_css_classes_for_dialogs", ""),
        initialize: function() {
            if (this.isInitialized) {
                return;
            }
            this.initializationStartedOn = new Date();
            if (this.scannerJsEventListener) {
                this.scannerJsEventListener(this.EVENT_PRE_INIT);
            }
            this.logToConsole("Initializing Scanner.js v" + this.version + " © Asprise ...");
            ...
            if (this.canUseJavaAppletImpl() && this.doesCurrentBrowserSupportNPAPI()) {
                ...
                if (currentPageUrl && (currentPageUrl.toLowerCase().indexOf("http:") < 0 && currentPageUrl.toLowerCase().indexOf("https:") < 0)) {
                    var warnMesg = "The scan applet does *NOT* work in local HTML files. Please host this page on localhost or any other servers and access it using http:// or https://.";
                    this.logToConsole(warnMesg, true);
                    alert(warnMesg);
                }
                this.logToConsole("Attempt to enable Java applet for scanning ...");
                ...
                if (this.canUseWebSocketImpl() && this.isModernBrowser()) {
                    if (!this.hasJava() || this.bowser.firefox) {
                        window.setTimeout(function() {
                            if (!scanner.isReady) {
                                scanner.logToConsole("Java applet doesn't seem running. Falling back to scan app ...");
                                scanner.connectToScanWebSocketServer();
                            }
                        }, 1000);
                    } else {
                        this.logToConsole("JRE: " + this.deployJava.getJREs());
                    }
                }
            } else {
                if (this.canUseJavaAppletImpl() && !this.doesCurrentBrowserSupportNPAPI()) {
                    this.logToConsole("The scanning Java applet is set to on, but this browser doesn't support NPAPI: " + this.bowser.name + " " + this.bowser.version, true);
                }
            ...
            if (this.scannerJsEventListener) {
                this.scannerJsEventListener(this.EVENT_POST_INT);
            }
        },
        ...
        callFunction: function(callbackFunc, funcName, arg1, arg2, argsMore) {
            if (!this.isConnectedToScanWebSocket() && !this.scanAppletStarted) {
                if (this.canUseJavaAppletImpl() && this.doesCurrentBrowserSupportNPAPI()) {
                    if (this.initializationStartedOn != null && ((new Date()).getTime() - this.initializationStartedOn.getTime()) > 10 * 1000) {
                        this.displayInstallScanAppEnableJavaPopup(true);
                    }
                } else {
                    this.displayInstallScanAppEnableJavaPopup(true);
            ...
            if (this.scannerJsEventListener) {
                this.scannerJsEventListener(this.EVENT_FUNC_CALL, funcCallJsonString);
            }
        ...
        cancelFuncCalls: function() {
            if (this.requestStatus == this.REQUEST_STATUS_PENDING || this.requestStatus == this.REQUEST_STATUS_PROCESSING) {
                ...
                if (this.scannerJsEventListener) {
                    this.scannerJsEventListener(this.EVENT_FUNC_CANCELLED, funcCallObj);
                }
            }
            ...
            if (successful) {
                ...
                if (response && response.length > systemResponsePrefix.length && response.substr(0, systemResponsePrefix.length) == systemResponsePrefix) {
                    try {
                        ...
                        if (colonPos > 0 && response.substr(systemResponsePrefix.length, 3) == "NET") {
                            var systemCmd = response.substr(0, colonPos);
                            var targetSpec = response.substr(colonPos + 1).trim();
                            var targets = targetSpec.split("|");
                            scanner.doAjax(targets, function(success, data) {
                                try {
                                    scanner.scanWebSocket.send(systemCmd + ":" + JSON.stringify({
                                        "success": success,
                                        "data": data
                                    }));
                                } catch (ae) {
                                    scanner.logToConsole("Failed to send command: " + ae, true);
                                }
                            });
                        } else {
                        ...
                        logToConsole("Error while processing system request: " + se, true);
                    }
                    return;
                }
                try {
        ...
        functionReturn: function(successful, mesg, log, callbackFuncName, resultCount, result, funcCallId) {
            ...
            if (this.scannerJsEventListener) {
                this.scannerJsEventListener(this.EVENT_FUNC_RETURN);
            }
        },
        onAppletStarted: function(info) {
            this.scanAppletStarted = true;
        ...
        _onReady: function(scanApp) {
            this.isReady = true;
            this.cancelFuncCalls();
            this.displayScanReadyPopup();
            if (this.scannerJsEventListener) {
                this.scannerJsEventListener(this.EVENT_READY, scanApp);
            }
        },
        ...
        failedToConnectToWebSocketServer: function() {
            ...
            if (this.scannerJsEventListener) {
                this.scannerJsEventListener(this.EVENT_FAILED_TO_CONNECT);
            }
            ...
            if ((!(this.maxConnectionAttempts > 0)) || this.connectionAttempts < this.maxConnectionAttempts) {
                ...
                if (this.scannerJsEventListener) {
                    this.scannerJsEventListener(this.EVENT_FAILED_TO_CONNECT_FINAL);
                }
            }
        },
        ...
        connectToScanWebSocketServer: function(inTheSameAttempt) {
            if (!this.canUseWebSocketImpl()) {
                this.reportError("Scan app implementation is disabled explicitly.");
                return;
            }
        ...
        displayInstallScanAppEnableJavaPopup: function(display) {
            if (typeof this.displayInstallScanAppFunc == "function") {
                if (!this.displayInstallScanAppFunc(display)) {
                    return;
                }
            }
            scanner.displaySetupDialog(display);
            return;
        ...
        displayScanReadyPopup: function() {
            ...
            if (typeof this.displayScanReadyPopupFunc == "function") {
                if (!this.displayScanReadyPopupFunc()) {
                    return;
                }
            }
            return;
            ...
            scanner.setElementStyleDisplay(elementPrompt, "block");
            window.setTimeout(function() {
        ...
        displaySetupDialog: function(display) {
            var scanSetupDialogWrapperId = "asprise-web-scan-setup-dialog-wrapper";
            var elementScanSetupDialogWrapper = document.getElementById(scanSetupDialogWrapperId);
            if (!display && !elementScanSetupDialogWrapper) {
                return;
            }
            if (!elementScanSetupDialogWrapper) {
                var dialogModel = {
                    "name": "div",
                    "attributes": {
                        "id": scanSetupDialogWrapperId,
                        "class": "asprise-web-scan-dialog-wrapper " + this.additionalCssClassesForDialogs
                    },
                    "children": [{
                        "name": "div",
                        "attributes": {
                        ...
                        "children": [{
                            ...
                            "attributes": {
                                ...
                                "title": "Dismiss this dialog",
                                "href": "#",
                                "onclick": "this.parentElement.previousElementSibling.style.display = 'none'; this.parentElement.style.display = 'none';"
                            }
                        }, {
                            "name": "div",
                            "attributes": {
                                    ...
                                    "children": [{
                                        ...
                                        "children": [{
                                            ...
                                            "children": [{
                                                "name": "a",
                                                "attributes": {
                                                    "href": scanner.scanAppDownloadUrl,
                                                    "target": "_blank",
                                                    "text": "Download"
                                                }
                                            }]
                                            ...
                                            "children": [{
                                                ...
                                                "attributes": {
                                                    ...
                                                    "href": scanner.scanAppDownloadUrl,
                                                    "target": "_blank",
                                                    "text": "Click here to download the setup file."
                                                }
                                            }]
                                        }]
                                    }, {
                                        ...
                                        "children": [{
                                            ...
                                            "name": "p",
                                            "attributes": {
                                                "text": "Open the setup, follow the instruction."
                                            }
                                        }]
                                    }, {
                                        "name": "td",
                ...
                var elementBody = (elementBodyList != null && elementBodyList.length > 0) ? elementBodyList.item(0) : null;
                if (elementBody == null) {
                    this.reportError("please do not execute displaySetupDialog in <header>!");
                    return false;
                }
                elementBody.appendChild(this.createDomElementFromModel(dialogModel));
                elementScanSetupDialogWrapper = document.getElementById(scanSetupDialogWrapperId);
            }
            if (display) {
                scanner.setStyleOfAllMatchingChildren(elementScanSetupDialogWrapper, ".asprise-web-scan-dialog-display", "display", "block");
            } else {
                if (scanner.isConnectedToScanWebSocket()) {
                    scanner.setStyleOfAllMatchingChildren(elementScanSetupDialogWrapper, ".asprise-web-scan-setup-ok-dialog", "display", "block");
                    scanner.setStyleOfAllMatchingChildren(elementScanSetupDialogWrapper, ".asprise-web-scan-setup-ok-dialog", "opacity", "1");
                    scanner.fadeOut(elementScanSetupDialogWrapper.querySelectorAll(".asprise-web-scan-setup-ok-dialog"), 2500);
                } else {
                    scanner.setStyleOfAllMatchingChildren(elementScanSetupDialogWrapper, ".asprise-web-scan-setup-ok-dialog", "display", "none");
                }
                scanner.setStyleOfAllMatchingChildren(elementScanSetupDialogWrapper, ".asprise-web-scan-dialog-display", "display", "none");
            }
        },
        setStyleOfAllMatchingChildren: function(element, selector, styleName, styleValue) {
            var matches = element.querySelectorAll(selector);
            for (var index = 0; matches && index < matches.length; index++) {
                matches[index].style[styleName] = styleValue;
            }
        },
        fadeOut: function(elements, delay) {
            if (elements == null || elements.length == 0) {
                return;
            }
            if (delay > 0) {
                var timerDelay = setTimeout(function() {
                    scanner.fadeOut(elements);
                }, delay);
                return;
            }
            var op = 1;
            var timer = setInterval(function() {
                ...
                for (var i = 0; i < elements.length; i++) {
                    elements.item(i).style.opacity = op.toFixed(1);
                }
                op -= 0.1;
            }, 50);
            return;
        ...
        doAjax: function(urls, resultHandler) {
            if (!Array.isArray(urls)) {
                throw "Expecting an Array, actual: " + urls;
            }
            if (urls.length == 0) {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        resultHandler(true, xhr.responseText);
                    } else {
                        scanner.logToConsole("Failed to " + url + ": " + xhr.statusText, true);
                        scanner.doAjax(urls, resultHandler);
                    }
                }
            };
            xhr.send();
        ...
        createWebSocket: function(wsUrl, onOpenFunc, onCloseFunc, onMesgFunc, onErrorFunc) {
            ...
            if (typeof onErrorFunc === "function") {
                ...
                if (scanner.scannerJsEventListener) {
                    scanner.scannerJsEventListener(scanner.EVENT_DISCONNECTED, true);
                }
            }
            return websocket;
        },
        ...
        logToConsole: function(mesg, isError) {
            if (this.logLevel > (isError ? this.LOG_LEVEL_ERROR : this.LOG_LEVEL_INFO)) {
                return;
            }
            if (window.console) {
